      ...
      <div className="space-y-5">
        {isLoading ? (
          ...
          <>
            ...
            {/* SENSOR GAIB (Ditarik lebih rapat ke kartu terakhir) */}
            <div ref={bottomBoundaryRef} className="h-1 w-full" aria-hidden="true"></div>

            {/* INDIKATOR BATAS BAWAH (Margin negatif untuk menariknya ke atas) */}
            <div className={`-mt-4 pb-1 text-center transition-all duration-500 ease-in-out ${showEndIndicator ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
              <div className="inline-flex items-center justify-center gap-2 px-4 py-1.5 rounded-full bg-secondary/80 border border-border shadow-sm backdrop-blur-sm">
                <CheckCircle className="w-3.5 h-3.5 text-primary" />
                <span className="text-[11px] font-semibold text-muted-foreground">Semua berita telah dimuat</span>
              </div>
            </div>
          </>
        ) : (
